...
#include"dataset.h"
#include"assert.h"
using map_map_int = std::unordered_map<std::wstring, std::unordered_map<std::wstring, int>>;
using map_map_double = std::unordered_map<std::wstring, std::unordered_map<std::wstring, double>>;
using map_int=std::unordered_map<std::wstring, int>;
class HMMPosTag
{
public:
	HMMPosTag() = default;
	...
	void fit(const std::string &path,double alpha)
	{
		...
		for (int i=0; i< train.sentences.size(); i++)
		{
			...
			{
				//发射频数
				current_word = train.sentences[i].word[j];
				current_tag = train.sentences[i].tag[j];
				...
				{
					std::unordered_map<std::wstring, int> correspoding_tag{ {current_tag,1 } };
		...
		{				//转移频数
			...
			{
				current_tag = train.sentences[i].tag[j + 1];
				...
				{
					std::unordered_map<std::wstring, int> correspoding_tag{ {former_tag,1 } };
	...
	{//计算概率
		...
		for (auto w = word_tag_count.begin(); w != word_tag_count.end(); ++w)
		{
			...
			{
				prob = (w0->second + alpha) / double(all_tag[w0->first] + all_word * alpha);
			...
			for (auto w = all_tag.begin(); w != all_tag.end(); w++)
			{
				...
				{
					prob = alpha / double(w->second + all_word * alpha);
		...
		for (auto w = current_former_count.begin(); w != current_former_count.end(); w++)
		{
			...
			{
				//prob = (first_tag[w->first] +alpha)/ double(all_first_tag + first_tag.size() * alpha);
			//	prob = (first_tag[w->first]) / double(all_first_tag );
				prob = (first_tag[w->first] + alpha) / double(all_first_tag + all_word * alpha);
				other.emplace(L"start", prob);
			}
			else
			{
				//	prob =  alpha / double(all_first_tag + first_tag.size() * alpha);
				prob = alpha / double(all_first_tag + all_word * alpha);
			...
			for (auto w1 = all_tag.begin(); w1 != all_tag.end(); w1++)
			{
				...
				{
					prob = alpha / double(w1->second + all_word * alpha);
		...
		for (auto z = first_tag.begin(); z != first_tag.end(); z++)
		{
			...
			{
				std::unordered_map<std::wstring, double> other;
				other.emplace(L"start", (z->second + alpha) / double(all_first_tag + all_word * alpha));
				for (auto z = all_tag.begin(); z != all_tag.end(); z++)
				{
					if (other.find(z->first) == other.end())
					{
						other.emplace(z->first, alpha / double(z->second + all_word * alpha));
					}
				}
	...
	{
		std::vector<std::vector<double>> prob(words.size(), vector<double>(tag.size(), (std::numeric_limits<double>::min)()));
		std::vector<std::vector<int>> path(words.size(), vector<int>(tag.size(), -1));
		//处理第一个词:
		...
		for (int i = 1; i < words.size(); i++)
		{
			...
			for (auto launch_tag_prob = current_word_tag->second.begin(); launch_tag_prob != current_word_tag->second.end(); launch_tag_prob++)
			{
				std::vector<double> local_prob(tag.size());
		...
		int index = distance(prob[words.size() - 1].begin(), w);
	...
	void appraise(const std::string &file_name)
	{
		...
		cout << "标注词性的准确率为" << correct << "/" << sens.word_count << "=" << correct / double(sens.word_count) << endl;
	}
private:
	std::unordered_map<std::wstring, std::unordered_map<std::wstring, double>> transfer;
	std::unordered_map<std::wstring, std::unordered_map<std::wstring, double>> launch_prob;
	std::unordered_map<std::wstring, int> tag;
	std::unordered_map<int, std::wstring> vector_tag;
